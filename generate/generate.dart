...
Library generateLibrary(Map<Category, SparseBoolList> categories,
        SparseList<int> characters, CaseMapping caseMapping) =>
    Library((b) => b
      ..name = 'unicode'
...
Spec generateVariableCategories(SparseList<int> characters) =>
    generateSparseListInt(
      ...
      compressGroups(characters.groups
...
List<Spec> generateIsCategoryFunctions() => Category.abbrToCategory.values
    .map((category) => generateIsCategory(
        category.name.pascalCase, getCharacterSetName(category)))
    .toList();
